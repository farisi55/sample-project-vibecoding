...
language: English
changes_from: v1.7.9 — New conditional flag external_assets (P01, alongside project_shape/simple_mode) generates a companion asset-manifest.md for projects with human-supplied photos/logos/fonts the agent can't create itself; §6 gets a one-line pointer to it, §9 gets a missing-asset fallback rule, and Step 3 (P03 + P04) cross-checks it for UI tasks — all omitted entirely when external_assets: false, so shapes without a UI (backend-api, cli-worker) are untouched. A new Asset manifest check, siblings to but separate from the Knowledge drift check, resolves the same task an unlisted asset is discovered — same immediate-not-deferred principle as v1.7.9
---
