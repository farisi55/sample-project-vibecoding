...
language: English
changes_from: v1.7.10 — PRD (P00) gains §4.6 Brand & Visual Identity (color palette, typography, brand voice, visual direction; omit if no UI), appended after §4.5 so §5-11 keep their numbers. knowledge.md (P01) §6's single vague "typography and color token rules" bullet becomes a structured Design Tokens sub-block extracted from PRD §4.6, with the same extract-to-a-separate-file-if-it-grows-large escape hatch §7 already has for schema.md — default stays inline, since the audience is the agent applying values, not a human sourcing files (unlike asset-manifest.md)
---
