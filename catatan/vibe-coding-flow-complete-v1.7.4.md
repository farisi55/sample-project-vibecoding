...
language: English
changes_from: v1.7.3 — 4 findings applied (Terminal vs Routing shape taxonomy clarified; vestigial architecture_pattern field removed; Scope & Boundaries added; cli-worker description broadened)
---
...
Usually no. Whether you edit the existing prd.md or start a fresh one at Prompt 00
depends on whether the *problem being solved* is still recognizably the same one,
not on how much has changed in absolute terms.
...
project has become?
- `ALIGNED` — trajectory still serves the original Problem/Solution/Success Metric
- `DRIFTED` — cumulative changes have shifted the project away from its original
  intent, even if every individual change was reasonable and fully documented.
  DRIFTED is not automatically wrong — see Appendix B Scenario 8 for how to
  respond: either update PRD §1 to honestly reflect the new direction, or
  course-correct [NEXT TASKS] back toward the original intent. The point of this
  check is to make that a deliberate choice rather than something that happened
  by accumulation with no one deciding it.
