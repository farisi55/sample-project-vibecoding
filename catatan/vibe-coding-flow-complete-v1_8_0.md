...
language: English
changes_from: v1.7.12 — New optional knowledge.md §8 field "Code graph tool" (none | graphify | codegraph, manual — not PRD-inferred), wired into Step 3 (P03 + P04) as a preference for MCP structural queries over grep/broad reads. Minor version bump (not 1.7.13): marks a checkpoint after six consecutive hardening rounds since v1.7.6 (hard-stop, branch strategy, drift-resolution timing, asset manifest, brand/design tokens, changelog structure verification), at the developer's request, rather than reflecting unusual size in this specific change
---
...
>
> **v1.7.7 changes:** Prompt 04's loop is no longer fully automatic — Step 7 (Loop
> Decision) now hard-stops after every task and requires a manual P04 re-invocation
> to proceed, regardless of how many items remain in [NEXT TASKS]; the Loop
> Contract and Role description are updated to match, since "a continuous task
> execution loop" no longer accurately describes behavior that requires human
> sign-off after each task. The Regression Gate (P03 Step 7, P04 Step 5) now
> requires showing the actual test command and a pass/fail summary rather than
> accepting a narrative "tests pass" claim — closing a gap where a task could
> reach [COMPLETED] on an unverifiable assertion. This is a deliberate trade of
> throughput for auditability: P04 keeps the Impact Analysis, Context Window
> Guard, and Health Report apparatus P03 lacks, but no longer differs from P03 in
> whether it pauses for review — only in what it checks along the way. Appendix A's
> prompt-selection guidance is updated to match, since "want full automation" was
> no longer an accurate reason to reach for P04. Note: the section name
> "Progressive Looping Task Executor" is now partly a misnomer since nothing loops
> without a human restarting it; left unchanged pending a deliberate rename
> decision.
>
> **v1.7.8 changes:** P04's branch strategy no longer lets feat/* branches nest
> across the whole task queue — Step 3 now runs `git checkout dev && git pull
> origin dev` before cutting `feat/task-[NNN]-[title-slug]`, so every task
> branches from the shared trunk instead of the previous task's tip. Step 6
> gains a merge-and-cleanup sub-step: on PASSED, the branch is merged into `dev`
> with `--no-ff`, pushed, and deleted (local + remote) before the next task is
> promoted, rather than deferring every merge to a single bulk PR at the very
> end of the project as the prior wording implied. A conflict at this step is
> labeled MERGE BLOCKED, not FAILED — the task's own gates already passed, only
> the integration needs Appendix C's Merge Conflict Resolution, whose rebase
> target is updated from `main` to `dev` to match. `main` stays outside the
> loop entirely: promoting `dev` → `main` remains a manual, developer-reviewed
> PR, matching the convention P03 already used — the two prompts no longer
> disagree on what `main` means. Loop Contract wording updated to state this
> explicitly.
>
> **v1.7.9 changes:** Knowledge drift (P03 + P04) is no longer batched until the
> next Phase Transition — the moment Step 6 (P04) / Step 8 (P03) answers YES to
> any drift trigger, `knowledge.md` §[N] is edited that same task, its version
> bumped, and `knowledge_version` in `changelog.md` synced to match, instead of
> leaving tasks later in the same Phase to work against a stale document. Drift
> triggers grow from 8 to 9 with "new top-level module/folder ∉ §3," detected
> off the Forward File Impact scan already run in Step 1. Phase Transition
> Check's "all drift tags resolved" item is kept but demoted to a safety net,
> since it should now already be trivially true. Separately, `knowledge.md` §4
> gains a one-line docstring/comment requirement on every exported function,
> class, and method, and §9 gains a short, selective Sensitive/High-Blast-Radius
> Code list — both wired into Step 3's execution checklist. Neither section
> attempts a full function/class map: that level of detail drifts almost every
> task, and a wrong map is worse than none, so the durable parts (structure,
> what NOT to touch carelessly) stay in `knowledge.md` while per-function detail
> stays where it can't go stale — next to the code itself.
>
> **v1.7.10 changes:** New conditional flag `external_assets` (P01, alongside
> `project_shape`/`simple_mode`) generates a companion `asset-manifest.md` for
> projects with human-supplied photos, client/venue logos, or licensed fonts
> the agent can't create itself — grouped by category, seeded from the PRD,
> using a naming pattern plus examples rather than one row per near-identical
> item. `knowledge.md` §6 gets a one-line pointer to it; §9 gets a
> missing-asset fallback rule (placeholder/graceful degradation required,
> never a broken build); Step 3 (P03 and P04) cross-checks it for UI tasks.
> Everything here is omitted when `external_assets: false`, so shapes with no
> UI (backend-api, cli-worker) are completely untouched. A new Asset manifest
> check runs alongside the Knowledge drift check in Step 6 (P04) / Step 8
> (P03) — a separate check, targeting a separate file, but resolved the same
> task an unlisted asset is discovered, matching v1.7.9's immediate-not-
> deferred principle rather than reintroducing a batched-flag problem for a
> second file.
>
> **v1.7.11 changes:** Traced a gap the developer found in a real project (no
> .md anywhere held color/typography/tone decisions) back past `knowledge.md`
> to the PRD template itself — none of PRD's 11 sections captured brand or
> visual identity, so §6's one-line "typography and color token rules" bullet
> had nothing to extract even when written carefully. PRD (P00) gains
> `§4.6 Brand & Visual Identity` (color palette, typography, brand voice,
> visual direction, existing brand guide reference; omit if no UI), appended
> after §4.5 so §5 through §11 keep their numbers. `knowledge.md` (P01) §6's
> vague bullet becomes a structured Design Tokens sub-block extracted from
> PRD §4.6, mirroring §7's Data Schema treatment down to the same
> extract-to-a-separate-file-if-it-grows-large escape hatch (`design-
> tokens.md`, same pattern as `schema.md`). Deliberately NOT treated like
> `asset-manifest.md`: the audience here is the agent applying values
> directly, not a human sourcing files, so it stays inline in `knowledge.md`
> by default rather than starting as a separate file.
>
> **v1.7.12 changes:** Fixed a real failure found on a live project — Task #001
> completed, but the Promote step's changelog edit left `[NEXT TASKS]`'s header
> gone entirely (its 25 remaining tasks stranded under `[IN PROGRESS]`) and a
> duplicate `[COMPLETED]` header at the end of the file. Comparing the P04 text
> across the two versions in use at the time showed zero differences, so this
> wasn't a prompt regression — it's a class of mechanical slip Promote's
> multi-part edit (cut from NEXT TASKS, paste into IN PROGRESS, append to
> COMPLETED, all in one pass) can produce regardless of version, the same way
> a claimed file write can be wrong regardless of version. New Changelog
> Structure Verification (P03 Step 8, P04 Step 6) runs right after Promote,
> before the commit: exactly one of each section header, exactly one task in
> `[IN PROGRESS]` (unless the project just finished, when zero is correct),
> and every task number accounted for exactly once across the three sections.
> Failure is labeled CHANGELOG STRUCTURE FAILED, kept distinct from FAILED,
> since the task's own gates already passed — this is a bookkeeping break, not
> a reason to redo the work.
>
> **v1.8.0 changes:** Bumped from the 1.7.x patch series to a new minor version
> at the developer's request — a checkpoint marking six consecutive hardening
> rounds since v1.7.6, not a reflection of unusual size in this specific
> change (on diff size alone it would have been v1.7.13). Content: after
> comparing code-graph tools (Graphify, CodeGraph) for cutting agent token
> usage on structural questions — and confirming both names are shared by
> several unrelated projects with different reliability, so "which one" needs
> to be recorded precisely — `knowledge.md` §8 gains an optional "Code graph
> tool" field (`none` / `graphify` / `codegraph`, naming the exact repo/package
> installed). Deliberately NOT inferred from the PRD like `project_shape`/
> `simple_mode`/`external_assets`: which tool (if any) fits is a developer-
> workflow choice, not a project requirement, so the field is set manually,
> defaulting to `none`. Adopting one mid-project needs no new mechanism —
> it's already an Infra ∉ §8 drift trigger from v1.7.9, resolved the same
> task per that principle. Step 3 (P03 and P04) gains one line: when §8
> names a tool, prefer its MCP structural-query tools over grep/broad reads
> before falling back to direct file reads.
...
```
  ...
  │             │  → One gated task per run — stops for manual review, then
  │             │    re-run to continue
  │             │  → Final Project Health Report (when queue empties)
  └──────┬──────┘
         │
         ▼
...
**Step 6 — Run Tasks via P04** *(varies)*
Run Prompt 04. It executes one task, then stops for your review — re-run it to
work through [NEXT TASKS] one task at a time. Final Health Report generated when
the queue empties.
...
### 4.6 Brand & Visual Identity (omit this section entirely if this shape has no UI)

- **Color palette:** [primary / secondary / accent — hex codes if the client
  already has them, otherwise "TBD — design phase" is acceptable; note what
  each color is used for, e.g. "primary #E85D3D — CTAs and active states"]
- **Typography:** [font family or family stack per weight used, e.g. "Human
  Sans — Regular 400 body, SemiBold 600 headings"; type scale if defined]
- **Brand voice / tone:** [2-4 descriptive adjectives, e.g. "friendly,
  playful, family-first"; one or two do's/don'ts for UI copy, e.g. "avoid
  corporate or formal phrasing"]
- **Visual direction:** [overall mood/theme, e.g. "playful, rounded, colorful"
  vs. "minimal, sharp, corporate"; light/dark theme support if applicable]
- **Existing brand guide:** [link or reference if the client already has one;
  "none — define during design phase" otherwise]
- **Scale note:** if this grows into a full design system (spacing scale,
  shadow/elevation styles, icon set, animation curves), consider a separate
  design-tokens.md referenced from knowledge.md instead of inlining all of it
  — see @knowledge §6 Design Tokens guidance, same pattern as §7 Data Schema.
...
# Compute external_assets
Set external_assets: true if BOTH are true, else false:
  - project_shape is frontend-only, fullstack, mobile, or desktop (has a UI)
  - PRD's branding/UI material references real-world assets the agent cannot
    generate itself: client- or venue-supplied photography, client/venue logos,
    licensed/custom fonts, or similarly sourced visual assets
If true: also generate asset-manifest.md (template below) alongside @knowledge,
seeded from whatever the PRD already enumerates — group by asset category, use
a naming pattern plus 2-3 examples for repeated items (e.g. 30+ similar photo
sets), never one row per near-identical item.
State the reasoning inline as a comment near the frontmatter, e.g.:
  # external_assets: true — PRD references client activity photos and 29 client/venue logos
...
- Docstring/comment requirement: one-line purpose on every exported function,
  class, and method. This is the source of truth for what code does — it lives
  next to the code it describes instead of duplicated into this document, so it
  can't quietly go stale the way a separate function-level map would
...
### Design Tokens
Extract from PRD §4.6 Brand & Visual Identity — copy actual values (hex codes,
font names, weights) rather than re-summarizing into "uses the design system."
This is what every UI task in Step 3 references for consistent styling; vague
entries here are why a component ends up with an ad-hoc color no other screen
uses.

- **Color palette:** [primary/secondary/accent hex codes + usage, from PRD §4.6]
- **Typography:** [font family/stack per weight, from PRD §4.6]
- **Brand voice / tone:** [from PRD §4.6 — for any UI copy the agent writes]
- **Visual direction:** [overall mood/theme, light/dark support if applicable]
- If this grows into a full design system (spacing scale, shadow/elevation,
  icon set, animation curves): extract into a separate `design-tokens.md` file
  at the project root, with this subsection replaced by a one-line pointer
  ("See design-tokens.md — last updated [date]"). Same pattern as §7's
  schema.md — proportional to how much design detail this project actually has,
  not a default for every project.
...
- External asset requirements: see `asset-manifest.md` (omit this line if
  external_assets: false — most projects don't need this file at all)
...
- Code graph tool: none | graphify | codegraph — record the exact repo/package
  installed (e.g. "codegraph — ctype-lab/codegraph"), not just the generic
  name; several unrelated projects share both names, with different reliability.
  Manual field, set by the developer — not inferred from PRD like project_shape/
  simple_mode/external_assets, since the choice is about developer workflow
  style, not a project requirement. Default: none. Rough guide: none for
  simple_mode or small projects; graphify for projects heavy on non-code
  reference material (PRDs, brand guides, asset-manifest.md) or that fit this
  flow's per-task checkpoint rhythm; codegraph for continuous/rapid ad-hoc
  coding outside the P04 loop. Adopting one mid-project is Infra ∉ §8 drift —
  update this line the same task it's installed, per the existing drift check.
...
- Missing-asset fallback policy (omit if external_assets: false): every
  component referencing a human-supplied asset (photo, client/venue logo,
  licensed font) must render a graceful placeholder or fallback — never a
  broken image, layout break, or failed build — until the real asset lands

### Sensitive / High-Blast-Radius Code (populate as discovered — starts empty)
Short, selective list, not a function inventory. Only entries where a careless
change has outsized consequences: regulatory/financial calculations, code
called from many sites, or logic that breaks silently. Format: `function/class`
in `path` — one-line reason.

Example:
- `calculateInterest()` in `src/finance/interest.js` — implements the
  regulatory formula; do not change without legal review
- `AuthMiddleware.verify()` in `src/middleware/auth.js` — called from 12
  routes; a change here has repo-wide blast radius

# Companion file — asset-manifest.md (only if external_assets: true)
Generate alongside @knowledge, at the project root — never as a section inside
@knowledge itself, since its audience is a human sourcing files, not an agent
making architectural decisions. Seed it from whatever the PRD already
enumerates; leave it out entirely for any project where external_assets: false.

Group by asset category (fonts, hero/page images, repeated per-item photo
sets, client/venue logos, etc). For any category with more than ~5 near-
identical items (e.g. 30+ activity photo sets), use a naming pattern plus 2-3
concrete examples and state how many more follow it — never one row per item.

Per entry: `filename | format | dimensions (px, or - if not applicable) | path
| description`. For each category, state the fallback behavior required by §9
above, so it's clear nothing breaks while the real asset is still missing.

Add a short closing section for non-image content the project also needs from
a human (real testimonials, legal copy, placeholder figures) — same "agent
cannot supply this" checklist, just not a file asset.

# Quality Criteria (self-check before saving)
- [ ] Frontmatter includes project_shape, simple_mode, and external_assets,
      each with stated reasoning
...
- [ ] §6 (UI) present only if this shape has a UI; Design Tokens copied from
      PRD §4.6 with actual values (hex codes, font names) — not omitted or left
      as "see design system" when §4.6 has real data to extract
...
- [ ] All KNOWLEDGE UPDATE REQUIRED tags from previous phase resolved (safety
      net — Step 8's drift check now resolves drift the same task it's found,
      so this should already be trivially true; a failure here means a step
      was skipped, not that resolution is expected here)
...
- §6 UI (if applicable): SRI for CDN-loaded scripts/styles (web); platform UI/
  accessibility guidelines (mobile/desktop); if external_assets: true and this
  task renders a human-supplied asset, use the exact filename/path convention
  from `asset-manifest.md` and implement its fallback per §9
...
- §4 Code Standards: naming, formatting, test isolation; one-line docstring/
  comment on every exported function, class, or method touched or added
- §9 Anti-patterns: all forbidden patterns avoided, including non-constant-time
  ...
  local storage for tokens (mobile/desktop); cross-check the Sensitive/High-
  Blast-Radius list before modifying any function or class named there — treat
  a match as a signal to slow down and confirm scope, not a hard block
- §8 Code graph tool (if not "none"): prefer its MCP query tools (dependency
  graph, call graph, impact analysis) over grep/broad file reads for structural
  questions, before falling back to direct reads
...
Phase 2+: run the project's test command (e.g. npm test / pytest / go test ./... —
  per @knowledge §2 or §8). Show the exact command and a pass/fail summary (e.g.
  "42 passed, 0 failed, 3.2s") in the working output — a narrative claim of "all
  tests pass" without shown output does NOT satisfy this gate. All previously
  passing tests must still pass; no test fails due to shared state from new tests.
Phase 6: also verify coverage on domain/service layer meets @knowledge §4 target,
  against the actual coverage report — not an estimate.
...
If any test fails: this task is FAILED — do not proceed to Step 8.
...
Move first [NEXT TASKS] item to [IN PROGRESS]. Copy exactly. Do NOT execute this
newly promoted task in this run — this prompt is single-shot; re-run P03 or P04
separately when ready to work on it.
If [NEXT TASKS] empty: set completion note, update status: complete.

### Changelog Structure Verification (after Promote, before Bump)
After the append-to-[COMPLETED] and Promote edits above, verify @changelog's
structure before it's committed:
- [ ] Exactly one `## [COMPLETED]`, one `## [IN PROGRESS]`, one `## [NEXT TASKS]`
      header exists — never zero, never duplicated. An empty section keeps its
      header with an explicit "(none — ...)" placeholder; it is never removed.
- [ ] `[IN PROGRESS]` holds exactly one task, UNLESS `[NEXT TASKS]` is also
      genuinely empty (0 tasks) — the only state where 0 is correct, meaning
      the project just finished its last task.
- [ ] Every task number from the original total appears in exactly one of
      [COMPLETED] / [IN PROGRESS] / [NEXT TASKS] — none missing, none duplicated
      across sections.

If any check fails → CHANGELOG STRUCTURE FAILED: [which check, what's wrong] —
do not proceed to Bump / commit. Reconstruct the three sections from whatever
they currently (incorrectly) hold, then resume this step. The task's own gates
already passed — this is a changelog-bookkeeping problem, not a code problem.
...
### On PASSED — Knowledge drift check (all phases) — resolved immediately, not deferred
...
- Introduced a new top-level module/folder NOT reflected in @knowledge §3
...
YES to any → edit @knowledge §[N] now, this task, to reflect the change; bump
its version; sync `knowledge_version` in @changelog to match. Then record
`Knowledge drift: UPDATE REQUIRED: @knowledge §[N] — [what changed]` in the
COMPLETED entry — the tag documents an edit already made, not a pending one.
NO to all → `Knowledge drift: none`

### On PASSED — Asset manifest check (only if external_assets: true)
YES if this task's UI references a human-supplied asset (photo, client/venue
logo, licensed font, or similar) not yet listed in `asset-manifest.md`.
YES → add the entry now, this task — filename convention, format, dimensions,
path, description, and the fallback behavior this task's code implements for
it — same file, same immediate-not-deferred principle as the knowledge drift
check above, just targeting `asset-manifest.md` instead of `@knowledge`.
NO / external_assets: false → `Asset manifest: none` / `Asset manifest: N/A`
...
executing one gated task per invocation. All gates are non-negotiable, adapted to
this project's shape and scale per @knowledge frontmatter.
...
# Loop Contract
- Execute ONE task from [IN PROGRESS], then STOP — do not proceed to the next
  task in this same run, regardless of how many items remain in [NEXT TASKS].
  A human re-runs P04 to work on the next task.
...
- EVERY passing task merges into `dev` and deletes its feat/* branch before the
  next task starts — dev is the only branch this loop touches; promoting dev to
  main is a separate, manual PR the developer reviews and merges personally,
  never automated by this loop
...
- [ ] All KNOWLEDGE UPDATE REQUIRED tags from previous phase resolved (safety
      net — Step 6 now resolves drift the same task it's found, so this should
      already be trivially true; a failure here means a step was skipped, not
      that resolution is expected here)
- [ ] Zero open TODO: Task #[NNN] comments where NNN ≤ current task number
...
NEW MODULE CHECK: if this task's files sit under a top-level folder not already
listed in @knowledge §3's folder/module structure, note it — feeds the Step 6
knowledge drift check (new top-level module/folder ∉ §3).
...
- §6 UI (if applicable): SRI for CDN assets (web); platform UI guidelines
  (mobile/desktop); if external_assets: true and this task renders a human-
  supplied asset, use the exact filename/path convention from
  `asset-manifest.md` and implement its fallback per §9
...
- §9 Anti-patterns: all avoided, including non-constant-time secret comparison,
  ...
  cross-check the Sensitive/High-Blast-Radius list before modifying any function
  or class named there — treat a match as a signal to slow down and confirm
  scope, not a hard block
- §8 Code graph tool (if not "none"): prefer its MCP query tools (dependency
  graph, call graph, impact analysis) over grep/broad file reads for structural
  questions, before falling back to direct reads
...
Phase 2+: run the project's test command (e.g. npm test / pytest / go test ./... —
  per @knowledge §2 or §8). Show the exact command and a pass/fail summary (e.g.
  "42 passed, 0 failed, 3.2s") — a narrative claim without shown output does NOT
  satisfy this gate. All previously passing tests pass; no shared-state failures.
Phase 6: coverage on domain/service layer meets @knowledge §4 target, against the
...
If any test fails: this task is FAILED — do not proceed to Step 6.
...
### Changelog Structure Verification (after Promote, before Bump + Commit)
After the append-to-[COMPLETED] and Promote edits above, verify @changelog's
structure before it's committed:
- [ ] Exactly one `## [COMPLETED]`, one `## [IN PROGRESS]`, one `## [NEXT TASKS]`
      header exists — never zero, never duplicated. An empty section keeps its
...
If any check fails → CHANGELOG STRUCTURE FAILED: [which check, what's wrong] —
do not proceed to Bump + Commit or Merge to dev. Reconstruct the three sections
from whatever they currently (incorrectly) hold, then resume this step. The
task's own gates already passed — this is a changelog-bookkeeping problem, not
a code problem.
...
### On PASSED — Merge to dev + cleanup
```
git checkout dev
git merge --no-ff feat/task-[NNN]-[title-slug] -m "Merge task #[NNN] into dev"
git push origin dev
git branch -d feat/task-[NNN]-[title-slug]
git push origin --delete feat/task-[NNN]-[title-slug]
```
Merge conflict → MERGE BLOCKED: [files] — the task's own gates already passed;
this is a git-integration conflict, not a task failure. Do not mark FAILED and
do not discard the branch. Resolve per Appendix C Merge Conflict Resolution
(target dev, not main), then resume this step.

### Knowledge drift check (9 triggers, all phases) — resolved immediately, not deferred
Library ∉ §2 · Naming ≠ §4 · Pattern ∉ §3 · New top-level module/folder ∉ §3
(per Forward File Impact above) · API URL ≠ §5 · Error handling ≠ §4 · Infra ∉ §8
· Delete ≠ §7 · Test isolation ≠ §4

YES → edit @knowledge §[N] now, this task, to reflect the change. Bump
@knowledge's version and sync `knowledge_version` in @changelog to match — do
not defer to the next Phase Transition Check. Then record `UPDATE REQUIRED:
@knowledge §[N] — [what changed]` — the tag documents an edit already made
this task, not a pending one.
NO → `none`

### Asset manifest check (only if external_assets: true)
YES if this task's UI references a human-supplied asset (photo, client/venue
logo, licensed font, or similar) not yet listed in `asset-manifest.md`.
YES → add the entry now, this task — filename convention, format, dimensions,
path, description, and the fallback behavior this task's code implements for
it — same file, same immediate-not-deferred principle as the knowledge drift
check above, just targeting `asset-manifest.md` instead of `@knowledge`.
NO / external_assets: false → `none` / `N/A`

## Step 7 — Loop Decision
This task PASSED all gates and is recorded in [COMPLETED]. Output the Iteration
Summary below, then STOP unconditionally — do NOT proceed to Step 1 for the next
task, regardless of how many items remain in [NEXT TASKS]. A human reviews the
result and re-runs P04 manually to execute the next task.

[NEXT TASKS] empty → skip the Iteration Summary, go straight to Final Project
Health Report → stop.
...
─────────────────────────────────────────────────────────────
 ...
 Next:          Task #[NNN] — [title]
 Action:        STOPPED — review this result, then re-run P04 to continue
─────────────────────────────────────────────────────────────
...
| Task #001 passed, continuing through the queue with full tracking (impact analysis, context guard, health reports) | P04 |
...
**Scenario 4: Drift tags found unresolved (should be rare since v1.7.9 — Step 6
resolves drift the same task it's found)**
If it happens anyway (imported changelog, manual edit, a step skipped): collect
the outstanding UPDATE REQUIRED tags → edit knowledge.md → bump version →
update knowledge_version in changelog → continue P04.
...
```
1. git checkout feat/task-[NNN]-[slug] && git rebase dev
...
| 1.7.7 | 2025-06-28 | 1 feature request applied, prompted by the developer wanting a hard boundary between AI-driven execution and human sign-off — Regression Gate already blocked [COMPLETED] on failing tests, but nothing made the test evidence itself visible, and P04's Loop Decision auto-continued through the entire task queue with no review point between tasks: |
| | | **Added:** P04's Step 7 (Loop Decision) now hard-stops after every task and requires a manual re-invocation to proceed to the next one, regardless of how many items remain in [NEXT TASKS] — Role and Loop Contract wording updated to match, since "a continuous task execution loop" no longer described behavior that pauses for human sign-off after each task. Regression Gate (P03 Step 7, P04 Step 5) now requires showing the exact test command and a pass/fail summary rather than accepting a narrative "tests pass" claim. This is a deliberate throughput-for-auditability trade: P04 retains the Impact Analysis, Context Window Guard, and Health Report apparatus P03 lacks, but the two prompts no longer differ in whether they pause for review — only in what they track along the way. Appendix A's prompt-selection guidance updated to match, since "want full automation" was no longer an accurate reason to reach for P04. |
| 1.7.8 | 2025-06-28 | 1 feature request applied, prompted by the developer noticing that 30+ sequential P04 tasks would each branch from the previous task's tip rather than a shared trunk — Step 3 only ever ran `git checkout -b` with no merge-back step anywhere in the loop, so feat/* branches nested instead of isolating, and the only merge instruction in the whole prompt was a bulk "merge feat/* via PR" recommendation in the Final Project Health Report, deferred until every task was already done: |
| | | **Added:** Step 3 now runs `git checkout dev && git pull origin dev` before cutting `feat/task-[NNN]-[title-slug]`, so every task branches from the latest trunk instead of the previous task's tip. Step 6 gains a new sub-step after Bump + Commit: on PASSED, the branch is merged into `dev` with `--no-ff`, pushed, and deleted (local + remote) before the next task is promoted. A conflict at this step is labeled MERGE BLOCKED rather than FAILED, since the task's own gates already passed and only the integration step needs Appendix C's Merge Conflict Resolution (rebase target updated from `main` to `dev` to match). `main` is deliberately untouched by the loop: promoting `dev` → `main` stays a manual, developer-reviewed PR, the same convention P03 already used — the two prompts no longer disagree on what `main` means. Loop Contract wording updated to state this explicitly. |
| 1.7.9 | 2025-06-28 | 2 feature requests applied, both prompted by the developer's goal of letting the agent orient from `knowledge.md` instead of scanning the repo, to save tokens — surfaced by tracing what actually happens to an `UPDATE REQUIRED` tag after Step 6 writes it: the only place resolution was enforced was the next Phase Transition Check, and Appendix C's own recovery recipe said "collect UPDATE REQUIRED tags" (plural), confirming tags were expected to accumulate across every task in a Phase before being addressed — the same shape of gap as v1.7.8's branch nesting, just for `knowledge.md` instead of git: |
| | | **Added:** Knowledge drift check (P03 Step 8, P04 Step 6) now resolves in the same task it's detected — edit `knowledge.md` §[N], bump its version, sync `knowledge_version` in `changelog.md`, all before the COMPLETED entry is written, instead of deferring to the next Phase Transition. Trigger count goes from 8 to 9: "new top-level module/folder ∉ §3," fed by a new NEW MODULE CHECK line in P04's Forward File Impact scan (Step 1). Phase Transition Check's "all drift tags resolved" item is kept as a safety net rather than the enforcement point, and Appendix C's "Drift accumulated" scenario is reframed as a rare fallback rather than the expected path. |
| | | **Added:** `knowledge.md` §4 (Code Standards) gains a one-line docstring/comment requirement on every exported function, class, and method. §9 (Constraints & Anti-patterns) gains a short, selective "Sensitive / High-Blast-Radius Code" list — not a function inventory, only entries where a careless change has outsized consequences. Both are wired into Step 3's execution checklist (P03 and P04) so they're actually consulted, not just present in the template. Deliberately excluded: a full function/class-level map. That granularity would drift on nearly every task — and a wrong map produces false confidence exactly where the developer wanted caution, which is worse than no map at all — so per-function detail stays in the code (as docstrings) rather than duplicated into a second, higher-maintenance source of truth. |
| 1.7.10 | 2025-06-28 | 1 feature request applied, prompted by the developer's real-world use of a hand-authored ASSET_MANIFEST.md on a client site (photos, client/venue logos, and licensed fonts the agent cannot generate) to avoid the agent guessing at filenames or paths for assets a human has to physically source — the request was to fold this into the workflow itself without affecting shapes that have no such need: |
| | | **Added:** P01 computes a third frontmatter flag, `external_assets`, alongside `project_shape`/`simple_mode` — true only when the shape has a UI AND the PRD references real-world assets the agent can't generate. When true, P01 also generates a companion `asset-manifest.md` at the project root (never a `knowledge.md` section — different audience, human sourcing files rather than agent making architecture decisions), grouped by category and seeded from the PRD, using a naming-pattern-plus-examples format for repeated items rather than one row each. `knowledge.md` §6 gets a one-line pointer to it; §9 gets a new missing-asset fallback rule requiring graceful placeholder behavior instead of a broken build. Step 3 (P03 and P04) cross-checks the manifest's naming/path convention whenever a task renders a human-supplied asset. All of this is a no-op when `external_assets: false`, so backend-api and cli-worker projects see no new template content at all. |
| | | **Added:** an Asset manifest check running alongside the Knowledge drift check in P04 Step 6 / P03 Step 8 — a separate check, targeting a separate file, since an unlisted asset isn't an architecture-drift concern — but resolved on the same immediate-not-deferred principle v1.7.9 established for `knowledge.md`, specifically to avoid reintroducing a second "flag now, integrate at some later checkpoint" mechanism for a second file. |
| 1.7.11 | 2025-06-28 | 1 feature request applied, prompted by the developer noticing a real project had no .md anywhere holding color/typography/tone decisions — tracing it found the cause one layer deeper than `knowledge.md`: none of PRD's 11 sections captured brand or visual identity at all, so §6's single "typography and color token rules" bullet had nothing to extract even if it were written with more structure, per P01's own no-fabrication rule: |
| | | **Added:** PRD (P00) gains `§4.6 Brand & Visual Identity` — color palette, typography, brand voice/tone, visual direction, existing brand guide reference — appended after §4.5 Data Model so §5 through §11 keep their existing numbers rather than triggering a renumbering pass. Omit entirely if the shape has no UI. |
| | | **Added:** `knowledge.md` (P01) §6's vague single bullet becomes a structured **Design Tokens** sub-block, extracted from PRD §4.6 with actual values (hex codes, font names) rather than a "see design system" placeholder — mirroring §7's Data Schema treatment down to the same extract-to-`design-tokens.md`-if-it-grows-large escape hatch already established for `schema.md`. Deliberately kept inline by default, unlike `asset-manifest.md`: the audience is the agent applying values directly to code, not a human sourcing physical files, so there's no reason to default to a separate file the way v1.7.10 did for assets. Quality Criteria gains a matching check: Design Tokens must carry real PRD §4.6 values, not be left vague or omitted when the source data exists. |
| 1.7.12 | 2025-06-28 | 1 bug found and fixed on a live project's `changelog.md` — Task #001 completed, but the Promote step's edit left `[NEXT TASKS]`'s header gone entirely (its remaining tasks stranded under `[IN PROGRESS]`, one of them duplicated) and a duplicate `[COMPLETED]` header at the end of the file. The developer initially suspected a v1.7.10→v1.7.11 regression; diffing P04's text across both versions showed zero differences, ruling that out — this is a mechanical slip Promote's multi-part edit can produce on any version, the same class of risk File Existence Verification already exists to catch for code files, just never extended to the changelog's own structure: |
| | | **Added:** Changelog Structure Verification (P03 Step 8, P04 Step 6), run immediately after Promote and before the commit (Bump / Bump + Commit). Confirms: exactly one `[COMPLETED]`/`[IN PROGRESS]`/`[NEXT TASKS]` header, always present even when a section is empty (an explicit "(none — ...)" placeholder, never a removed header); `[IN PROGRESS]` holds exactly one task unless `[NEXT TASKS]` is also genuinely empty (the one valid case for zero, meaning the project just finished); and every task number from the original total appears in exactly one section. Failure is labeled CHANGELOG STRUCTURE FAILED — deliberately distinct from FAILED, since the task's own gates already passed and this is a bookkeeping break, not a reason to redo the work. |
| 1.8.0 | 2025-06-28 | Minor version bump, at the developer's request — a checkpoint marking six consecutive hardening rounds since v1.7.6 (hard-stop, branch strategy, drift-resolution timing, asset manifest, brand/design tokens, changelog structure verification), not a reflection of this specific change's size (on diff size alone this would have been v1.7.13). Prompted by comparing code-graph tools (Graphify, CodeGraph) for cutting agent token usage on structural questions during development — both names turned out to be shared by several unrelated projects with materially different reliability, so recording exactly which one matters: |
| | | **Added:** `knowledge.md` §8 gains an optional "Code graph tool" field (`none` \| `graphify` \| `codegraph`, naming the exact repo/package installed rather than the bare generic name). Deliberately NOT inferred from the PRD the way `project_shape`/`simple_mode`/`external_assets` are: which tool (if any) fits is a developer-workflow choice, not a project requirement, so the field is set manually and defaults to `none`. A short rough guide ships in the field's own description (none for simple/small projects; graphify for reference-doc-heavy projects or ones that fit this flow's per-task checkpoint rhythm; codegraph for continuous ad-hoc coding outside the P04 loop) rather than a separate section. Adopting a tool mid-project needed no new mechanism — it's already an Infra ∉ §8 drift trigger from v1.7.9, resolved the same task per that principle. Step 3 (P03 and P04) gains one line: when §8 names a tool, prefer its MCP structural-query tools (dependency graph, call graph, impact analysis) over grep/broad file reads before falling back to direct reads. The "PROJECT LOOP COMPLETE" banner in the Final Project Health Report is updated from v1.7 to v1.8 to match. |
