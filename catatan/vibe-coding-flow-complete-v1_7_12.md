...
language: English
changes_from: v1.7.11 — New Changelog Structure Verification (P03 Step 8, P04 Step 6), run right after Promote and before the commit: confirms exactly one [COMPLETED]/[IN PROGRESS]/[NEXT TASKS] header, exactly one task in [IN PROGRESS] (unless the project just finished), and every task number accounted for exactly once — same verify-the-result philosophy as the existing File Existence Verification. Failures are labeled CHANGELOG STRUCTURE FAILED, distinct from FAILED, since the task's own gates already passed
---
