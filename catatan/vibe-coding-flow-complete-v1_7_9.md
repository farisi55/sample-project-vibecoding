...
language: English
changes_from: v1.7.8 — Knowledge drift (P03 + P04) now resolves the same task it's detected instead of batching at the next Phase Transition, with a 9th trigger for new top-level modules/folders; Phase Transition Check's drift item becomes a safety net. knowledge.md §4 gains a docstring/comment requirement and §9 gains a Sensitive/High-Blast-Radius Code list, both wired into Step 3's execution checklist — aimed at letting the agent orient from knowledge.md instead of scanning the repo, without duplicating function-level detail that would drift immediately
---
